...
2. В разделе "Аутентификация" выберите: **"Ничего" (Nothing)** ← это первый радиобаттон в списке
3. Сохраните

**⚠️ Важно:** Не выбирайте "API-ключ" (API-key) для публичного GPT - это требует токен от каждого пользователя. Выбирайте именно "Ничего" (Nothing).
...
2. В разделе "Аутентификация" выберите: **"API-ключ" (API-key)**
3. **Тип аутентификации (Auth Type):** выберите **"Bearer"** (не Basic, не Custom)
   - Это потому что токен передается как `Authorization: Bearer ghp_...`
4. **API-ключ:** Оставьте пустым или введите свой GitHub PAT для тестирования
   - Если пустое, каждый пользователь должен будет создать свой токен
